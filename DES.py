'''
 ...
 Requirements :file "message.txt"  - plain text to be Encrypted
			   ...
			   file "encypted.txt" -for only decryption option
 output : encrypted.txt -> has the ciphertext
		  decrypted.txt -> has the plaintext
 user Input : The program is designed to perform both encryption and decryption
	          or either of the two.The choice is taken from the user
			  Note : to perform only decryption file "encrypted.txt" should be present
'''
import sys
...
def generate_round_keys(encryption_key):
	''' This method 1.Shifts the key from previous round based on the key schedule
		...
		Method output : a list of 16 48-bit subkey for each round
	'''
...
def round_logic(left,right,logic,roundnumber):
	...
	'''
	newRight = right.permute( expansion_permutation ) 	##part of funtion f # changing 32 bit to 48 bit
	out_xor = newRight ^ round_keys[roundnumber]		##part of funtion f # xor ing key and text
	out_sbox=substitute(out_xor).permute(permutation)   ##part of funtion f -> substitue + pbox permutation
...
def encrypt():
	...
	bitvecs = divide_into_blocks(bv,64)
	for bitvec in bitvecs:
		if  len(bitvec) > 0:
			if len(bitvec) < 64: #last block of the msg might be less than 64bit
				bitvec.pad_from_right(64-len(bitvec))  #padding with zeros
			...
			asciistring= output.get_bitvector_in_ascii()  ##Readable!!
			ciphertext += asciistring
	with open("encrypted.txt","w",encoding="utf-8-sig",newline='') as wfile:
		wfile.write(ciphertext)
	return ciphertext

def decrypt():
	''' Decrypts the message using DES algorithm
		Method input: none
		Method output:Ciphertext
	'''
	plaintext=""
	key = get_encryption_key()
	round_keys = generate_round_keys( key )
	with open("encrypted.txt","r",encoding="utf-8-sig",newline='') as cf:
		message=cf.read()
	bv= BitVector(textstring = message)
	bitvecs = divide_into_blocks(bv,64) #dividing the message into 64 bit blocks
	for bitvec in bitvecs:
		if  len(bitvec) > 0:
			if len(bitvec) < 64:#last block of the msg might be less than 64bit
				bitvec.pad_from_right(64-len(bitvec)) #padding with zeros
			bitvec = bitvec.permute(initial_permuation)
			[LE, RE] = bitvec.divide_into_two()
			[FLE,FRE] = round_logic(LE,RE,"D",15) # D for decryption
			preoutput = FRE+FLE # 32 bit swap
			output = preoutput.permute(inverse_init_permutation) #inverse of initial permutation
			asciistring = output.get_bitvector_in_ascii()  ##Readable!!
			plaintext += asciistring
	with open("decrypted.txt","w",encoding="utf8") as wfile:
		wfile.write(plaintext)
	return plaintext
#############STARTS HERE###############
print("This program encrypts and decrypts a message according to DES algorithm")
print("Choose one of the following options")
print("1.To perform both encryption and decryption")
print("2.To perform encryption")
print("3.To perform decryption")
userinp=int(input())
...
if userinp is 1:
	...
	print("Invalid input, type number -> 1,2 or 3")
